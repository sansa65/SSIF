...
{
	$dn1 = mysqli_fetch_array(mysqli_query($connection,'select count(t.id) as nb1, t.title, t.parent, c.name from topics as t, categories as c where t.id="'.$id.'" and t.id2=1 and c.id=t.parent group by t.id'));
if($dn1['nb1']>0)
{
?>
...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        <div class="content">
<?php
$nb_new_pm = mysqli_fetch_array(mysqli_query($connection,'select count(*) as nb_new_pm from pm where ((user1="'.$_SESSION['userid'].'" and user1read="no") or (user2="'.$_SESSION['userid'].'" and user2read="no")) and id2="1"'));
...
if(isset($_POST['message']) and $_POST['message']!='')
{
	...
	$message = mysqli_real_escape_string($connection,bbcode_to_html($message));
	if(mysqli_query($connection,'insert into topics (parent, id, id2, title, message, authorid, timestamp, timestamp2) select "'.$dn1['parent'].'", "'.$id.'", max(id2)+1, "", "'.$message.'", "'.$_SESSION['userid'].'", "'.time().'", "'.time().'" from topics where id="'.$id.'"') and mysqli_query($connection,'update topics set timestamp2="'.time().'" where id="'.$id.'" and id2=1'))
	{
	?>
...
?>
		...
		<div class="foot"><a href="index.html">Back to HomePage</a></div>
	</body>
</html>
<?php
}
