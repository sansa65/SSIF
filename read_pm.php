...
if(isset($_POST['message']) and $_POST['message']!='')
{
	...
	$message = mysqli_real_escape_string(nl2br(htmlentities($connection,$message, ENT_QUOTES, 'UTF-8')));
	if(mysqli_query($connection,'insert into pm (id, id2, title, user1, user2, message, timestamp, user1read, user2read)values("'.$id.'", "'.(intval(mysqli_num_rows($req2))+1).'", "", "'.$_SESSION['userid'].'", "", "'.$message.'", "'.time().'", "", "")') and mysqli_query('update pm set user'.$user_partic.'read="yes" where id="'.$id.'" and id2="1"'))
	{
?>
