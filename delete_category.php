...
{
$id = intval($_GET['id']);
$dn1 = mysqli_fetch_array(mysqli_query($connection,'select count(id) as nb1, name, position from categories where id="'.$id.'" group by id'));
if($dn1['nb1']>0)
{
...
{
	if(mysqli_query($connection,'delete from categories where id="'.$id.'"') and mysqli_query($connection,'delete from topics where parent="'.$id.'"') and mysqli_query($connection,'update categories set position=position-1 where position>"'.$dn1['position'].'"'))
	{
	?>
