...
{
	$dn1 = mysqli_fetch_array(mysqli_query($connection,'select count(t.id) as nb1, t.authorid, t2.title, t.message, t.parent, c.name from topics as t, topics as t2, categories as c where t.id="'.$id.'" and t.id2="'.$id2.'" and t2.id="'.$id.'" and t2.id2=1 and c.id=t.parent group by t.id'));
if($dn1['nb1']>0)
{
...
if(isset($_POST['message']) and $_POST['message']!='')
{
	if($id2==1)
	{
		if($_SESSION['username']==$admin and isset($_POST['title']) and $_POST['title']!='')
		{
			...
			$title = mysqli_real_escape_string($connection,$dn1['title']);
		}
		else
		{
			$title = mysqli_real_escape_string($connection,$dn1['title']);
	...
	$message = mysqli_real_escape_string(bbcode_to_html($connection,$message));
	if(mysqli_query($connection,'update topics set title="'.$title.'", message="'.$message.'" where id="'.$id.'" and id2="'.$id2.'"'))
	{
	?>
